...
- **Queue-wide pause (shipped 1.1.10):** the header's `Pause queue` / `Resume queue` toggle is the "Pause all / Resume all" pair from §4.1, collapsed into one stateful button. Paused = nothing starts and running rows return to pending with progress kept (a pause, never a failure); persisted, so it survives a restart, and Settings → *Queue on launch → Start paused* makes every launch begin paused. The collapsed strip shows a `queue paused` chip. Palette: "Queue: pause/resume".
- **Row selection + bulk cancel (shipped 1.1.10):** rows select with the pane conventions (§3.6: click, Ctrl+click toggle, Shift+click range over the current sort order). `Cancel selected (n)` appears in the header and `Delete` does the same; `Escape` clears. `Cancel all queued` acts by *state* on the backend, not by id, so it reaches rows beyond the list window and rows queued while the dialog was open. `Cancel selected` confirms only when part-transferred data is at stake, mirroring single-row cancel; `Cancel all queued` always confirms because it is one click on a header button.
